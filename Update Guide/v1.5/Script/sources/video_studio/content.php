...
if (isset($_POST['type']) && !empty($_POST['type'])) {
    ...
    elseif ($_POST['type'] == 'likes') {
    	$limit = 20;
        $top_likes = $db->rawQuery('SELECT video_id, COUNT(*) AS count FROM '.T_DIS_LIKES.' l WHERE (SELECT id FROM '.T_VIDEOS.' WHERE user_id = '.$pt->user->id.' AND id = l.video_id) = video_id AND type = 1  GROUP BY video_id ORDER BY count DESC LIMIT '.$limit);
        if (!empty($top_likes)) {
    ...
    elseif ($_POST['type'] == 'dislikes') {
        $limit = 20;
        $top_dislikes = $db->rawQuery('SELECT video_id, COUNT(*) AS count FROM '.T_DIS_LIKES.' l WHERE (SELECT id FROM '.T_VIDEOS.' WHERE user_id = '.$pt->user->id.' AND id = l.video_id) = video_id AND type = 2  GROUP BY video_id ORDER BY count DESC LIMIT '.$limit);
    ...
    elseif ($_POST['type'] == 'comments') {
        ...
        $top_comments = $db->rawQuery('SELECT video_id, COUNT(*) AS count FROM '.T_COMMENTS.' c WHERE (SELECT id FROM '.T_VIDEOS.' WHERE user_id = '.$pt->user->id.' AND id = c.video_id) = video_id GROUP BY video_id ORDER BY count DESC LIMIT '.$limit);
