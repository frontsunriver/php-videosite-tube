...
if ($page == 'trending') {
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('time', time() - 172800, '>')->where('is_movie',0)->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('is_movie',0)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('is_movie',0)->orderBy('views', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
    ...
    if (!empty($_GET['id'])) {
        if (in_array($_GET['id'], array_keys($categories))) {
            ...
            // pagination system 
            $videos = $db->where('privacy', 0)->where('category_id = "' . $cateogry.'"')->where('is_movie',0)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
            $pt->total_pages = $db->totalPages;
            // pagination system 
