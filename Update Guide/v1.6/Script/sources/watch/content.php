...
$related_videos    = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE MATCH (title) AGAINST ('$query_video_title') AND id <> '{$get_video->id}' $not_in_query AND is_movie = 0 AND privacy = 0 ORDER BY `id` DESC LIMIT 20");
...
if ($pt->config->ffmpeg_system == 'on') {
    ...
    if ($pt->get_video->sell_video == 0 || PT_IsAdmin() || $pt->get_video->is_owner || ($pt->get_video->sell_video > 0 && $pt->is_paid > 0 && $pt->config->sell_videos_system == 'on')) {
        ...
        $get_video->video_location = $pt->config->site_url.'/'.$get_video->demo;
    }
}
else{
