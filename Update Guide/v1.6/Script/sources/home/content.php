...
if ($pt->theme_using == 'default') {
    $video_obj = $db->where('featured', '1')->where('is_movie', 0)->where('privacy', 0)->orderBy('RAND()')->get(T_VIDEOS,3);
    foreach ($video_obj as $key => $video) {
        $pt->videos_array[] = PT_GetVideoByID($video, 0, 1, 0);
    }
}
else{
    $video_obj = $db->where('featured', '1')->where('is_movie', 0)->where('privacy', 0)->orderBy('RAND()')->getOne(T_VIDEOS);
...
if (!empty($pro_users)) {
    ...
    $db->where('privacy', 0);
    $trending_data = $db->where('is_movie', 0)->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);
}
...
if (empty($top_data)) {
    $db->where('privacy', 0);
    $top_data = $db->where('is_movie', 0)->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);
}
...
if (!empty($pro_users)) {
    ...
    $db->where('privacy', 0);
    $db->orderBy('id', 'DESC');
    $latest_data = $db->where('is_movie', 0)->get(T_VIDEOS, $limit);
}

if (empty($latest_data)) {
    $db->where('privacy', 0);
    $latest_data = $db->where('is_movie', 0)->orderBy('id', 'DESC')->get(T_VIDEOS, $limit);
}
...
foreach ($categories as $cat_key => $cat_name) {
    ...
    $db->where('privacy', 0);
    $db->orderBy('id', 'DESC');
    $pt->cat_videos = $db->where('is_movie', 0)->get(T_VIDEOS, 8);
