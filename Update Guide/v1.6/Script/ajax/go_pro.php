...
if (!empty($_GET['first']) && $_GET['first'] == 'pay_to_see') {
	if (!empty($_POST['video_id']) && is_numeric($_POST['video_id'])) {
		...
		if (!empty($video)) {
		    ...
		    $item->setName('Pay to view video')->setQuantity(1)->setPrice($total)->setCurrency($payment_currency);  
		    $itemList->setItems(array($item));    
		    $details->setSubtotal($total);
		    $amount->setCurrency($payment_currency)->setTotal($total)->setDetails($details);
