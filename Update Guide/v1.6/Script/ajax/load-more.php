...
if (empty($_GET['first']) || empty($_POST['last_id'])) {
   ...
   //              }
		...
		if ($request === true) {
	 	...
	 	if ($last_video->is_movie) {
	 		$get = $db->rawQuery('SELECT * FROM '.T_VIDEOS_TRSNS.' t WHERE `paid_id` >= '.$pt->user->id.' AND (SELECT id FROM '.T_VIDEOS.' WHERE id = t.video_id AND `is_movie` = 1) = video_id AND `id` < '.$tr_id.' ORDER BY id DESC LIMIT 20');
	 	}
	 	else{
	 		$get = $db->rawQuery('SELECT * FROM '.T_VIDEOS_TRSNS.' t WHERE `paid_id` >= '.$pt->user->id.' AND (SELECT id FROM '.T_VIDEOS.' WHERE id = t.video_id AND `is_movie` != 1) = video_id AND `id` < '.$tr_id.' ORDER BY id DESC LIMIT 20');
	 	}
		...
		elseif ($type == 'most_active') {
			$ids = implode(',', $_POST['channels_ids']);
			$last_view = PT_Secure($_POST['last_id']);
			$videos = $db->rawQuery('SELECT * FROM '.T_USERS.' WHERE id NOT IN ('.$ids.') AND active_time <> 0  AND active_time <= '.$last_view.' ORDER BY active_time DESC LIMIT 20');
		}
