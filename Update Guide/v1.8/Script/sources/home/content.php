...
if ($pt->theme_using == 'default') {
    $video_obj = $db->where('featured', '1')->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie', 0)->where('privacy', 0)->orderBy('RAND()')->get(T_VIDEOS,10);
...
foreach ($categories as $cat_key => $cat_name) {
    if (IS_LOGGED == true && !empty($pt->user->fav_category)) {
        if (in_array($cat_key, $pt->user->fav_category)) {
            ...
            $db->where('privacy', 0);
            $db->orderBy('id', 'DESC');
            $pt->cat_videos = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->get(T_VIDEOS, 10);
