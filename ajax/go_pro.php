...
if ($first == 'payu') {
	if (!empty($_POST['card_number']) && !empty($_POST['card_month']) && !empty($_POST['card_year']) && !empty($_POST['card_cvc'])) {
		...
		$arParams['ORDER_PNAME[0]'] = "Go Pro Upgrade To Pro";
		$arParams['ORDER_PRICE[0]'] = intval($pt->config->pro_pkg_price);
...
if ($first == 'payu_pay_to_see') {
	if (!empty($_POST['card_number']) && !empty($_POST['card_month']) && !empty($_POST['card_year']) && !empty($_POST['card_cvc']) && !empty($_POST['video_id']) && is_numeric($_POST['video_id'])) {
		$video = PT_GetVideoByID($_POST['video_id'], 0,0,2);
		if (!empty($video)) {
			$text = "";
			if (!empty($_POST['pay_type']) && $_POST['pay_type'] == 'rent' && !empty($video->rent_price)) {
			...
			require_once('assets/import/PayU.php');
			$arParams['ORDER_PNAME[0]'] = "Pay to watch video";
			$arParams['ORDER_PRICE[0]'] = $total;
			$arParams['CC_NUMBER'] = $_POST['card_number'];
			$arParams['EXP_MONTH'] = $_POST['card_month'];
			$arParams['EXP_YEAR'] = $_POST['card_year'];
			$arParams['CC_CVV'] = $_POST['card_cvc'];
...
if($first == 'payu_subscribe'){
	if (!empty($_POST['card_number']) && !empty($_POST['card_month']) && !empty($_POST['card_year']) && !empty($_POST['card_cvc']) && !empty($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0) {
		$user_id       = (!empty($_POST['user_id']) && is_numeric($_POST['user_id'])) ? PT_Secure($_POST['user_id']) : 0;
		$user = PT_UserData($user_id);
    	if (!empty($user) && $user->subscriber_price > 0) {
    		require_once('assets/import/PayU.php');
			$arParams['ORDER_PNAME[0]'] = "Pay to subscribe";
			$arParams['ORDER_PRICE[0]'] = $user->subscriber_price;
			$arParams['CC_NUMBER'] = $_POST['card_number'];
			$arParams['EXP_MONTH'] = $_POST['card_month'];
			$arParams['EXP_YEAR'] = $_POST['card_year'];
			$arParams['CC_CVV'] = $_POST['card_cvc'];
