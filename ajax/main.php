...
if ($first == 'monetization' && $pt->config->user_mon_approve == 'on') {
	...
	elseif ($post == true) {
		...
		if (!empty($_FILES["personal_photo"]["error"]) || ($_FILES["personal_photo"]["size"] > $pt->config->max_upload)) {
	        $max   = pt_size_format($pt->config->max_upload);
        	$error = ($lang->file_is_too_big .": $max");
	    }
	    if (!empty($_FILES["id_photo"]["error"]) || ($_FILES["id_photo"]["size"] > $pt->config->max_upload)) {
	        $max   = pt_size_format($pt->config->max_upload);
        	$error = ($lang->file_is_too_big .": $max");
	    } 
